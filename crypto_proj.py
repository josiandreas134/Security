...
class CryptoProject(object):
    ...
    def __init__(self):
        ...
        self.student_id = 'ctaylor308'  # for test_crypto_proj_1.py
    ...
    def crack_password_hash(self, password_hash, weak_password_list):
        ...
        password = 'abc'
        salt = '123'
        for pas in weak_password_list:
            for sal in weak_password_list:
                hashed_password = hashlib.sha256(pas.encode() + sal.encode()).hexdigest()
    ...
    def recover_msg(self, N1, N2, N3, C1, C2, C3):
        ...
        y2 = N//N2
        y3 = N//N3
        m1=self.egcd(y1,N1)*C1*y1
        m2=self.egcd(y2,N2)*C2*y2
        m3=self.egcd(y3,N3)*C3*y3
        m123=(m1+m2+m3)%N 
        m=self.root(m123)
        return m
